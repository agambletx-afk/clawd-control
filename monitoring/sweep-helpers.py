...
def token_anomaly():
    ...
    try:
        ...
        print_json({"status": "alert", "message": f"failed reading session file: {exc}", "assistant_turns": 0, "user_turns": 0, "session_file": target})
        return 1

    if assistant_turns > 5 and user_turns == 0:
        print_json({
            "status": "alert",
            "message": f"token anomaly: {assistant_turns} assistant turns, 0 user turns in 30min",
    ...
    print_json({
        "status": "ok",
        "message": f"token activity normal: assistant={assistant_turns}, user={user_turns} in 30min",
        "assistant_turns": assistant_turns,
        "user_turns": user_turns,
        "session_file": target,
    })
...
def outbound_rate():
    ...
    url = f"https://api.telegram.org/bot{token}/getUpdates?offset=-100"
    try:
        with urllib.request.urlopen(url, timeout=20) as response:
            payload = json.loads(response.read().decode('utf-8'))
    except Exception as exc:
        print_json({"status": "alert", "message": f"telegram getUpdates failed: {exc}", "message_count": 0})
        return 1
    ...
    message = f"outbound rate spike: {count} messages in 60min" if status == "alert" else f"outbound rate normal: {count} messages in 60min"
...
def file_hashes():
    ...
    try:
        ...
        print_json({"status": "alert", "message": f"failed reading baseline: {exc}", "missing_files": missing, "changed_files": []})
        return 1
...
def backup_critical_files():
    ...
    for source in CRITICAL_FILES:
        ...
        destination = os.path.join(CRITICAL_BACKUP_DIR, f"{base}.{today}")
        tmp_destination = destination + f".tmp.{os.getpid()}"
        shutil.copy2(source, tmp_destination)
        os.replace(tmp_destination, destination)
        copied.append(destination)

        pattern = os.path.join(CRITICAL_BACKUP_DIR, f"{base}.*")
    ...
    print_json({"status": "ok", "message": f"critical file backups refreshed ({len(copied)} copied)", "copied_files": copied})
    return 0
...
def main():
    ...
    if cmd in ('--help','-h','help',''):
        print('Usage: sweep-helpers.py [token-anomaly|outbound-rate|file-hashes|backup-critical-files]')
        return 0

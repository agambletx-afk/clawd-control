...
DEPLOY NOTE: `scripts/check-cli-usage.sh` now detects Claude Code auth via `$HOME/.claude/.credentials.json`. Redeploy the VPS copy at `/usr/local/bin/check-cli-usage.sh` to apply this fix.
